...
This repository currently supports only **captcha_type** `slide`, `gobang`, `icon` and `ai`. If the project gains enough attention,
I will
consider adding more risk types.

---

## How to Obtain the Captcha ID

To use this solver, obtain the **captcha_id** from a GeeTest request. The `/load` response supplies `captcha_type`, which the solver uses to select the appropriate solving strategy.
...
6. Look for the `captcha_id` parameter in the request details and copy it.
